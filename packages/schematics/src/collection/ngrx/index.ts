...
function reducerSpec(className: string, fileName: string, propertyName: string) {
  return `
...
function generateNxFiles(context: RequestContext) {
  return (host: Tree) => {
    ...
    host.overwrite(path.join(context.moduleDir, context.options.directory, `${context.featureName}.effects.spec.ts`), effectsSpec(n.className, n.fileName));
    host.overwrite(path.join(context.moduleDir, context.options.directory, `${context.featureName}.reducer.spec.ts`), reducerSpec(n.className, n.fileName, n.propertyName));
  };
}
