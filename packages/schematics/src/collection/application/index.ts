import {chain, externalSchematic, move, Rule, Tree, noop} from '@angular-devkit/schematics';
import {Schema} from './schema';
import * as ts from 'typescript';
import {addBootstrapToModule} from '@schematics/angular/utility/ast-utils';
import {insertImport} from '@schematics/angular/utility/route-utils';
import {addImportToModule, addImportToTestBed, insert, updateJsonInTree} from '../../utils/ast-utils';
import {wrapIntoFormat} from '../../utils/tasks';
import {toFileName} from '../../utils/name-utils';
import {offsetFromRoot} from "@nrwl/schematics/src/utils/common";
import {getWorkspacePath, replaceAppNameWithPath} from "@nrwl/schematics/src/utils/cli-config-utils";
...
function updateComponentTemplate(options: NormalizedSchema): Rule {
  return (host: Tree) => {
    const content = options.routing
      ...
      : staticComponentContent;
    host.overwrite(`${options.appProjectRoot}/src/app/app.component.html`, content);
  };
}
  ...
  return (host: Tree) => {
    return chain([
      updateJsonInTree(getWorkspacePath(host), json => {
        ...
        const fixedProject =  replaceAppNameWithPath(project, options.name, options.appProjectRoot);
      ...
      updateJsonInTree(`${options.appProjectRoot}/tsconfig.app.json`, json => {
        return {
          ...
          compilerOptions: {
            ...
            outDir: `${offsetFromRoot(options.appProjectRoot)}dist/out-tsc/${options.appProjectRoot}`
          }
        };
      }),
  ...
  return (host: Tree) => {
    return chain([
      updateJsonInTree(getWorkspacePath(host), json => {
        ...
        const fixedProject =  replaceAppNameWithPath(project, options.e2eProjectName, options.e2eProjectRoot);
      ...
      updateJsonInTree(`${options.e2eProjectRoot}/tsconfig.e2e.json`, json => {
        return {
          ...
          compilerOptions: {
            ...json.compilerOptions,
            outDir: `${offsetFromRoot(options.e2eProjectRoot)}dist/out-tsc/${options.e2eProjectRoot}`
          }
        };
      })
    ])(host, null);
...
export default function (schema: Schema): Rule {
  return wrapIntoFormat(() => {
    ...
    return chain([
      externalSchematic('@schematics/angular', 'application', {...options, routing: false}),
...
function normalizeOptions(options: Schema): NormalizedSchema {
  ...
  return {...options, name: appProjectName, appProjectRoot, e2eProjectRoot, e2eProjectName, parsedTags};
}
