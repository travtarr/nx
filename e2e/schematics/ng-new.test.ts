import {
  ...
  runCLI,
  updateFile,
  runCommand, copyMissingPackages
} from '../utils';
...
describe('Nrwl Workspace', () => {
  it(
    ...
    () => {
      ...
      expect(runCLI('test --project=my-dir-my-app --no-watch')).toContain('Executed 3 of 3 SUCCESS');

      // running tests for the lib
      expect(runCLI('test --project=my-dir-my-lib --no-watch')).toContain('Executed 2 of 2 SUCCESS');

      expect(runCLI('test')).toContain('SUCCESS');

      // e2e tests
      expect(runCLI('e2e --project=my-dir-my-app-e2e')).toContain('Executed 1 of 1 spec SUCCESS');
    },
    1000000
  );
